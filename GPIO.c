...
void static adcinit(void){
  ...
  BSP_Delay1ms(1); // I'm commenting this line to see what line 238 does - JA
...
void BSP_RGB_Init(uint16_t red, uint16_t green, uint16_t blue){
  ...
  //while((SYSCTL_PRTIMER_R&0x02) == 0){};// allow time for clock to stabilize //uncommenting this line to see what happens - JA
	
  BSP_Delay1ms(500);
  // dly = 0; //this is not being used anywhere? - JA
	// dly = 0; //this is not being used anywhere? - JA
  ...
  //while((SYSCTL_PRGPIO_R&0x02) == 0){};// wait for clock to stabilize  // uncommenting this line to see what happens - JA
	BSP_Delay1ms(1);
		// dly = 0; //this is not being used anywhere? - JA
  // dly = 0; //this is not being used anywhere? - JA
	// dly = 0; //this is not being used anywhere? - JA
  ...
  //while((SYSCTL_PRTIMER_R&0x08) == 0){};// allow time for clock to stabilize // uncommenting this line to see what happens - JA
	BSP_Delay1ms(1);// dly = 0; //this is not being used anywhere? - JA
  // dly = 0; //this is not being used anywhere? - JA
	// dly = 0; //this is not being used anywhere? - JA
  ...
  //while((SYSCTL_PRWTIMER_R&0x01) == 0){};// allow time for clock to stabilize  // uncommenting this line to see what happens - JA
	BSP_Delay1ms(1);// dly = 0; //this is not being used anywhere? - JA
  // dly = 0; //this is not being used anywhere? - JA
	// dly = 0; //this is not being used anywhere? - JA
...
void BSP_Accelerometer_Init(void){
  adcinit();
  SYSCTL_RCGCGPIO_R |= 0x00000008; // 1) activate clock for Port D
  //while((SYSCTL_PRGPIO_R&0x08) == 0){};// allow time for clock to stabilize // uncommenting this line to see what happens - JA
	
  BSP_Delay1ms(1);// dly = 1; //this is not being used anywhere? - JA
	// dly = 1; //this is not being used anywhere? - JA
...
void BSP_Accelerometer_Input(uint16_t *x, uint16_t *y, uint16_t *z){
  ...
  //while((ADC0_RIS_R&0x04)==0){};   // 2) wait for conversion done // uncommenting this line to see what happens - JA
...
void BSP_Microphone_Init(void){
  ...
  //while((SYSCTL_PRGPIO_R&0x10) == 0){};// allow time for clock to stabilize // uncommenting this line to see what happens - JA
	BSP_Delay1ms(1);
...
void static commonInit(const uint8_t *cmdList) {
  ...
  //while((SYSCTL_PRGPIO_R&0x23) != 0x23){};// allow time for clocks to stabilize // uncommenting this line to see what happens - JA
  ...
  //while((SYSCTL_PRSSI_R&SYSCTL_PRSSI_R2) == 0){}; // uncommenting this line to see what happens - JA  
...
void BSP_LCD_PlotPoint(int32_t data1, uint16_t color1){
  ...
  //if(data1 > 98){
   //data1 = 98;
	if(data1 > 83){ //Trying this number instead - JA
    data1 = 83;
    color1 = LCD_RED;
  }
  if(data1 < 0){
...
void BSP_LCD_PlotIncrement(void){
  ...
  //BSP_LCD_DrawFastVLine(TimeIndex + 11, 17, 100, PlotBGColor); // Commenting out this line to see what happens - JA
}
/* ********************** */
/*   End of LCD Section   */
/* ********************** */
...
void BSP_Clock_InitFastest(void){
  ...
  while((SYSCTL_RIS_R&SYSCTL_RIS_PLLLRIS)==0){}; // This line was originally commented out - JA
...
void BSP_PeriodicTask_Init(void(*task)(void), uint32_t freq, uint8_t priority){long sr;
  ...
  //while((SYSCTL_PRWTIMER_R&0x20) == 0){};// allow time for clock to stabilize // uncommenting this line - JA
...
void BSP_PeriodicTask_InitB(void(*task)(void), uint32_t freq, uint8_t priority){long sr;
  ...
  //while((SYSCTL_PRWTIMER_R&0x10) == 0){};// allow time for clock to stabilize // uncommenting out this line  - JA

  BSP_Delay1ms(1);  //dly1 = 10;
  //dly1 = 10;	
  BSP_Delay1ms(1);	
...
void BSP_PeriodicTask_InitC(void(*task)(void), uint32_t freq, uint8_t priority){long sr;
  ...
  while((SYSCTL_PRWTIMER_R&0x08) == 0){};// allow time for clock to stabilize // uncommenting this line to see what happens - JA
  //dly1 = 10;
  //dly1 = 10;
...
void BSP_Time_Init(void){long sr;
  ...
  SYSCTL_RCGCWTIMER_R |= 0x20;     // activate clock for Wide Timer5
  //while((SYSCTL_PRWTIMER_R&0x20) == 0){};// allow time for clock to stabilize // uncommenting this line to see what happens - JA
...
// There are two I2C devikkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk2clockces on the Educational BoosterPack MKII:
...
uint16_t static I2C_Recv2(int8_t slave){
  ...
  int retryCounter = 1;
  do{
    while(I2C1_MCS_R&I2C_MCS_BUSY){};// wait for I2C ready // uncommenting this line to see what happens - JA
    //dly1 = 10;
    //dly1 = 10;
    BSP_Delay1ms(500);	
    I2C1_MSA_R = (slave<<1)&0xFE;    // MSA[7:1] is slave address
